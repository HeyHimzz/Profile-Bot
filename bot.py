#This is a Open Source Code Based Personal message bot.
#Dont Forget To Give Credits to @HeyHimzz

from details import API_ID, API_HASH, BOT_TOKEN, OWNER, START_IMG, START_MSG, BUTTON_1, BUTTON_2, LINK_1, LINK_2                
from pyrogram import Client, filters
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, User, Message

Bot = Client(
     session_name="Perosnal-Message-Bot"
     api_id=API_ID,
     api_hash=API_HASH,
     bot_token=BOT_TOKEN
)
...
#Join  @BotDesk For Any help
            

@Bot.on_message(filters.command('submit') & filters.private)
async def report(bot, message):
        if message.reply_to_message:
                                  await bot.send_message(chat_id=OWNER, text=f"<b>❓New Message 🎉\n \n⚡Name: {message.from_user.mention}\n⚡User ID:</b> <code>{message.chat.id}</code>")
                                  await bot.forward_messages(chat_id=OWNER, from_chat_id=message.from_user.id, message_ids=message.reply_to_message.message_id)
                                  await message.reply_text("<b>✅ Your Feedback Successfully Submitted to the Owner</b>")
        else:
...
async def replyt(bot, message):
    if message.from_user.id == OWNER: 
               if message.reply_to_message:
                                    ...
                                    await bot.send_message(chat_id=userid, text=f"<b>An Owner is responded to your feedback ✨</b>")
                                    await bot.copy_message(chat_id=userid, from_chat_id=OWNER, message_id=message.reply_to_message.message_id)
...
async def send(bot, message):
    if message.from_user.id == OWNER: 
               if message.reply_to_message:
                                    ...
                                    await bot.copy_message(chat_id=chatid, from_chat_id=OWNER, message_id=message.reply_to_message.message_id)
...
async def pin(bot, message):
    if message.from_user.id == OWNER: 
               if message.reply_to_message:
                                    ...
                                    p=await bot.copy_message(chat_id=chatid, from_chat_id=OWNER, message_id=message.reply_to_message.message_id)
                                    await p.pin()
